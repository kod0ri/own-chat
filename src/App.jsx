...
function App() {
      ...
      if (data.joined_rooms) {
        const roomPromises = data.joined_rooms.map(async (roomId) => {
          const nameRes = await fetch(
            `https://matrix.org/_matrix/client/r0/rooms/${encodeURIComponent(
              roomId
            )}/state/m.room.name`,
            {
              headers: { Authorization: `Bearer ${token}` },
  ...
  const fetchMessages = async (isFreshLoad = false) => {
    ...
    const tokenParam =
      lastSyncToken && !isFreshLoad ? `&since=${lastSyncToken}` : "";
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.event_id) {
        setMessages((prev) => [
          ...prev,
          { id: data.event_id, body: msg, sender: userId },
        ]);
      } else {
  ...
  const inviteUserToRoom = async () => {
    ...
    try {
      const res = await fetch(
        `https://matrix.org/_matrix/client/r0/rooms/${roomId}/invite`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const joinRoom = async () => {
    ...
    try {
      const res = await fetch(
        `https://matrix.org/_matrix/client/r0/join/${encodeURIComponent(
          joinRoomId.trim()
        )}`,
        {
          method: "POST",
  ...
  const fetchRoomMembers = async (currentRoomId, currentToken) => {
    if (!currentToken || !currentRoomId) return; // [cite: 392]

    try {
      const res = await fetch(
        `https://matrix.org/_matrix/client/r0/rooms/${encodeURIComponent(
          currentRoomId
        )}/joined_members`, // [cite: 395]
        {
          headers: { Authorization: `Bearer ${currentToken}` }, // [cite: 398]
        }
      );
      const data = await res.json();
      ...
      if (data.joined) {
        ...
        const members = Object.entries(data.joined).map(([userId, info]) => ({
          userId,
          displayName: info.display_name || userId.split(":")[0].substring(1), // [cite: 405]
          avatarUrl: info.avatar_url,
        }));
        setRoomMembers(members); // [cite: 404]
      }
    } catch (e) {
      console.error("Error fetching room members:", e); // [cite: 409]
    }
  };
  ...
  const leaveRoom = async (roomIdToLeave) => {
    ...
    try {
      ...
      if (res.ok) { //
        ...
        // Оновлюємо стан React, видаляючи кімнату зі списку
        setRooms(prevRooms => prevRooms.filter(r => r.roomId !== roomIdToLeave)); //
        
        // Якщо ми вийшли з активної кімнати, очищуємо інтерфейс
  ...
  const kickUser = async (userIdToKick) => {
    //
    if (!confirm(`Викинути користувача ${userIdToKick} з кімнати?`)) { //
      return;
    }
    ...
    try {
      ...
      if (res.ok) { //
        ...
        // Оновлюємо стан React, видаляючи юзера
  ...
  useEffect(() => {
    if (accessToken && roomId) {
      setMessages([]);
      setLastSyncToken("");
      setRoomMembers([]); // <-- Очищуємо старих учасників
      fetchMessages(true);
      fetchRoomMembers(roomId, accessToken); // <-- ДОДАЙТЕ ЦЕ
    }
  }, [roomId, accessToken]);
